...
final class gz extends DragAndDrop.Source {
  gz(gb paramgb, Actor paramActor, int paramInt) {
    super(paramActor);
  }
  ...
  public final DragAndDrop.Payload dragStart(InputEvent paramInputEvent, float paramFloat1, float paramFloat2, int paramInt) {
    if (!this.y.q())
      return null; 
    Image image2;
    if ((image2 = this.y.C[this.dL - 1]) == null)
      return null; 
    DragAndDrop.Payload payload;
    (payload = new DragAndDrop.Payload()).setObject(new hq(false, this.dL));
    float f2 = ((cq)this.y.j).bP;
    n n;
    ...
    Image image1;
    (image1 = new Image((textureRegion != null) ? (Drawable)new TextureRegionDrawable(textureRegion) : (Drawable)this.y.i)).setScaling(Scaling.none);
    ...
    this.y.b.setDragActorPosition(-f1, -paramFloat1);
    return payload;
  }
}
